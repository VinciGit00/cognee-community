...
This directory features custom community pipelines, where anyone can define new pipelines if they wish.
...
When adding a new pipeline, one should follow these steps:
...
1) In the `pipeline` directory, create a new directory with the name of the new pipeline
    ...
    for the pipeline), such as `cognee_community_pipeline_codify`.
