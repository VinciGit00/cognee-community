...
class CodeRetriever(BaseRetriever):
    ...
    async def _process_query(self, query: str) -> "CodeRetriever.CodeQueryInfo":
        ...
        try:
            ...
            logger.info(
                f"LLM extracted {len(result.filenames)} filenames and {len(result.sourcecode)}"
                f" chars of source code"
            )
            return result
    ...
    async def get_retrieved_objects(self, query: Optional[str]) -> Any:
        ...
        if not files_and_codeparts.filenames or not files_and_codeparts.sourcecode:
            ...
            logger.info(
                f"Using extracted filenames ({len(files_and_codeparts.filenames)}) and source"
                f" code for targeted search"
            )
        ...
        logger.info(
            f"Total search results: {total_items} items ({len(similar_filenames)} filenames,"
            f" {len(similar_codepieces)} code pieces)"
        )
    ...
    async def get_completion_from_context(
        ...
        logger.info(
            f"Successfully read {len([f for f in retrieved_files.values() if f])} files"
